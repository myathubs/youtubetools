...
# Helper utilities for downloading audio, filenames, and logging
# ----------------------------------------
import os, sys
...
def save_as_md(file_path: str, content: str) -> None:
    ...
    logger.info(f"[MyatHubs] Saving content to {file_path}")
...
def download_youtube_audio(dir_path: str, url: str):
    try:
        ...
        logger.info(f"[MyatHubs] Audio downloaded successfully: {output_file_path}")
        return output_file_path

    except Exception as ex:
        logger.error(f"[MyatHubs] Audio download failed: {str(ex)}")
        return None
